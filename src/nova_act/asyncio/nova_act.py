...
from nova_act.asyncio.impl.dispatcher import ActDispatcher
from nova_act.asyncio.tools.actuator.interface.actuator import ActionType, ActuatorBase
from nova_act.asyncio.tools.browser.default.default_nova_local_browser_actuator import (
    DefaultNovaLocalBrowserActuator,
)
from nova_act.asyncio.tools.browser.default.playwright_instance_options import (
...
class NovaAct:
    ...
    async def __aenter__(self) -> NovaAct:
        await self.start()
        return self
    ...
    async def __aexit__(
        ...
        if not self.started:
            ...
            return
        await self._stop(exc_type=exc_type)
    ...
    async def go_to_url(self, url: str) -> None:
        ...
        await self._actuator.go_to_url(url)
        await self._actuator.wait_for_page_to_settle()
    ...
    async def _stop(self, exc_type: Type[BaseException] | None = None) -> None:
        try:
    ...
    async def _act(
        ...
        try:
            result = await self.dispatcher.dispatch(act)
