...
async def ensure_element_focus(page: Page, x: float, y: float, retries: int = 2) -> None:
    """
    ...
    for _ in range(retries):
        ...
        focus_state = await is_element_focused(page, x, y)
...
async def agent_type(
    ...
    if os.path.isfile(value):
        ...
        chooser = await click_and_maybe_return_file_chooser(
            page,
            x=point["x"],
            y=point["y"],
            timeout_ms=1500,
    ...
    try:
        await ensure_element_focus(page, point["x"], point["y"])
    ...
    if len(value) > 10:
        ...
        await page.keyboard.type(value, delay=100)  # Types slower, like a user

    if additional_options and additional_options == "pressEnter":
        await page.keyboard.press("Enter")


async def handle_color_input(page: Page, element_info: ElementDict, color_value: str) -> None:
    """
    ...
    try:
        ...
        await element.evaluate(f"""(element) => {{
    ...
    *,
    ...
    try:
        if file_input_element:
            ...
            try:
                await page.locator(file_input_element).first.set_input_files(file_path)
            except Exception:
...
async def handle_range_input(page: Page, element_info: ElementDict, range_value: str) -> None:
    """
