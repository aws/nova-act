...
from nova_act.asyncio.impl.program.runner import ProgramRunner, format_return_value
...
def _handle_act_fail(
    f: "Callable[[ActDispatcher, Act], Coroutine[object, object, ActGetResult]]",
) -> "Callable[[ActDispatcher, Act], Coroutine[object, object, ActGetResult]]":
    """Update Act objects with appropriate metadata on Exceptions."""

    @functools.wraps(f)
    async def wrapper(self: ActDispatcher, act: Act) -> ActGetResult:
        try:
            return await f(self, act)
...
class ActDispatcher:
    ...
    @_handle_act_fail
    async def dispatch(self, act: Act) -> ActGetResult:
        ...
        program_result = await self._program_runner.run(executable)
