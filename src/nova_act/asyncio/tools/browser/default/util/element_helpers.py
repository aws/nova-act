...
async def viewport_dimensions(page: Page) -> DimensionsDict:
    viewport = await page.evaluate(Expressions.GET_VIEWPORT_SIZE.value)
    return {"height": viewport["height"], "width": viewport["width"]}


async def blur(point: dict[str, float], page: Page) -> None:
    await page.evaluate(
        """
        ([x, y]) => {
            %s
...
async def recurse_through_iframes(page: Page, x: float, y: float, inner_js: str, return_type: type[_T]) -> _T | None:
    """
...
async def get_element_at_point(page: Page, x: float, y: float) -> ElementDict | None:
    """
...
async def check_if_native_dropdown(page: Page, x: float, y: float) -> bool:
...
async def is_element_focused(page: Page, x: float, y: float) -> FocusState:
    """
    ...
    result: dict[str, bool] = await page.evaluate(
        """
        ([x, y]) => {
            %s
            const elem = deepElementFromPoint(document, x, y);
